...
model_name = True # брать обозначение и наименование из модели ("False" - имя файла, "True" - обозначение и наименование из модели)
rewrite = False # перезаписывать файлы с одинаковыми именами ("False" - не перезаписывать, "True" - перезаписывать)

mass_saving = True # опция масового сохранения (для открытых файлов), ("False" или "" - не задаёт вопрос, сохраняет только текущий открытый файл, "True" - сохраняет все открытые вкладки с файлами)

file_version = "12" # версия в какую сохранять файл ("False" или "" - задавать вопрос при сохранении), примеры: "19"; "18,1"; "14.2"
file_version_name = True # запись версии в имя файла ("False" или "" - не записывать, "True" - записывать)

near_the_source = True # путь к папке (пример: "C:\ASCON"), куда сохранять файлы (для открытых файлов) ("False" или "" - задавать вопрос при сохранении, "True" - рядом с исходником)

source_directory = r"C:\Users\Каширских Дмитрий\Desktop\Дмитрий\ГОСТ\Прочее\Макросы\Сохранение в старые версии\Тест" # путь к папке (пример: "C:\ASCON"), откуда брать файлы (для сохранения из папки) ("False" или "" - задавать вопрос при сохранении)
final_directory = False # путь к папке (пример: "C:\ASCON"), куда сохранять файлы (для сохранения из папки) ("False" или "" - задавать вопрос при сохранении, "True" - рядом с исходником)
#-------------------------------------------------------------------------------
...
def File_search(directory): # поиск файлов (путь к папке для поиска)
    ...
    for n in type_doc: # перебор всех типов документов
...
##    current_file_name = "" # что бы избежать ошибки окна в потоке
    ...
    for file in allfilesPaths: # создаём цикл для работы с каждым файлом из списка
        ...
        if Stop == False: # если не нажата кнопка "Отмена" или крестик
            ...
            if isinstance(save, bool) == False: # если файл не сохранён
                error_files.append(save) # добавляем дет. в список
    ...
    file_number = file_number - len(error_files) # количество обработаных файлов без ошибок

    Message("Файлы сохранены!\nВсего файлов обработано: " + str(file_number)) # сообщение, поверх всех окон с автоматическим закрытием (текст, время закрытия)
...
def Save_file(mass_saving, Path): # сохраняем файл/файлы (True - массовое сохранение; False - одиночное, путь к папке куда сохранять файлы)
    ...
    Path = Path_or_question(Path, "Выберите папку сохранения файлов") # вписан путь или нужно задать путь к папке
...
def Saving_file(ver, mass_saving, Path): # сохраняем файл (версия файла, опция массового сохранения, путь к папке)
    ...
    if iKompasDocument1.SaveAsEx(resultPath, ver[1]): # сохранить в старую версию
        ...
        Message(f"Файл не может быть сохранён в v{str(ver[0])}\nПопробуйте исключить из расчёта дерево постоения.") # сообщение, поверх всех окон с автоматическим закрытием (текст, время закрытия)

        if os.path.exists(resultPath): # проверка существования пути
            os.remove(resultPath) # удаляем файл который создался но имеет 0 байт
...
def Open_file(error_files): # открываем файлы
...
##    current_file_name = "" # что бы избежать ошибки окна в потоке
    ...
    error_files.reverse() # список в обратном порядке

    all_failes_number = len(error_files) # количество всех файлов в списке

    Message_count(all_failes_number, "Идёт открытие файлов!") # выдача сообщений о количестве файлов (количество всех файлов, сообщение) + file_number (номер обрабатываемого файла) + current_file_name (текущее название файла)

    iApplication.HideMessage = KompasConst.ksHideMessageNo # скрыть сообщение перестроения

    for file in error_files: # создаём цикл для работы с каждым файлом
...
error_files = [] # список не сохранённых файлов
...
if iApplication.ActiveDocument: # проверяем открыт ли файл в КОМПАСе
    ...
    if iDocuments.Count > 1: # количество открытых вкладок
        ...
        if mass_saving: # если массовое сохранение включено
            if Askyesnocancel("Сохранить все документы?"): # вопросительное сообщение, поверх всех окон
                Save_file(True, near_the_source) # сохраняем файл/файлы (True - массовое сохранение; False - одиночное, путь к папке куда сохранять файлы)

            else: # сохраняем только один
                Save_file(False, near_the_source) # сохраняем файл/файлы (True - массовое сохранение; False - одиночное, путь к папке куда сохранять файлы)
    ...
    if len(error_files) > 0:
        if Askyesno(f"Есть не сохранённые файлы!\nВсего файлов не сохраненно:{len(error_files)}\nОткрыть их?"): # вопросительное сообщение, поверх всех окон
            Open_file(error_files) # открываем файлы