...
file_version = False # версия (примеры: "19"; "18,1"; "14.2") в какую сохранять файл ("True" - сохранять в v5.11; "False" или "" - задавать вопрос при сохранении)
    ...
    for file in allfilesPaths: # создаём цикл для работы с каждым файлом из списка
        ...
        if Stop == False: # если не нажата кнопка "Отмена" или крестик
            ...
            if iKompasDocument: # если документ открылся
                ...
                Message(f"Не удалось открыть файл!\n\"{file}\"", 6) # сообщение, поверх всех окон с автоматическим закрытием (текст, время закрытия)
                continue
...
def Open_file(error_files): # если есть не сохранённые файлы, задаём вопрос и открываем их
    ...
    if iDocuments.Count > 1: # количество открытых вкладок
        ...
        if mass_saving: # если массовое сохранение включено
            ...
            if Askyesnocancel("Сохранить все документы?"): # вопросительное сообщение, поверх всех окон
                ...
                Open_file(error_files) # если есть не сохранённые файлы, задаём вопрос и открываем их
